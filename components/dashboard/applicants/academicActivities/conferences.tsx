...
import { UseConferences } from "@/hooks/useApplication";
...
const conferencesSchema = z
  .object({
    hasConferences: z.enum(["Yes", "No"]),
    conferences: z.array(conferenceSchema),
  })
  .superRefine((data, ctx) => {
    // If hasConferences is "Yes", validate that at least one conference has a title and file
    if (data.hasConferences === "Yes") {
      const hasAtLeastOneValidConference = data.conferences.some(
        (c) => c.title && c.title.trim() !== "",
      );

      if (!hasAtLeastOneValidConference) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "At least one conference with a title is required",
          path: ["conferences"],
        });
      }

      // Also check if any conference has a file when conferences exist
      if (data.conferences.length > 0) {
        const hasFileForConference = data.conferences.every(
          (c) => c.file instanceof File,
        );
        if (!hasFileForConference) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Each conference must have a file",
            path: ["conferences"],
          });
        }
      }
...
type ConferencesFormData = z.infer<typeof conferencesSchema>;

interface Props {
  onChange?: (hasConferences: "Yes" | "No", conferences: Conference[]) => void;
}

// Icons
const Icons = {
  Conference: ({ className = "w-4 h-4" }) => (
    <svg
      className={className}
      viewBox="0 0 24 24"
      fill="none"
...
interface FormInputProps {
  label: string;
  type: string;
  register: ReturnType<UseFormRegister<ConferencesFormData>>;
  error?: string;
  placeholder: string;
...
const ConferencesForm: React.FC<Props> = ({ onChange }) => {
  const router = useRouter();
  const { data: userSession, status } = useSession();
  const mutation = UseConferences();

  // State to track if conferences are selected
  const [conferencesSelected, setConferencesSelected] = useState(false);
  ...
  const {
    ...
    control,
    trigger,
  } = useForm<ConferencesFormData>({
    resolver: zodResolver(conferencesSchema),
    defaultValues: {
      hasConferences: "No",
      conferences: [],
    },
  ...
  const hasConferences = watch("hasConferences");
  const conferences = watch("conferences");

  // Effect to handle hasConferences change
  useEffect(() => {
    if (!hasConferences) {
      setConferencesSelected(false);
      setValue("conferences", []);
      return;
    }

    setConferencesSelected(true);

    // Initialize with one empty conference if "Yes" is selected and no conferences exist
    if (hasConferences === "Yes" && fields.length === 0) {
      append({ title: "", file: undefined as unknown as File });
    }
  }, [hasConferences, setValue, append, fields.length]);

  // Handle file changes separately (not through react-hook-form)
  const handleFileChange = (index: number, file: File) => {
    const currentConferences = getValues("conferences");
    const updatedConferences = [...currentConferences];
    updatedConferences[index] = { ...updatedConferences[index], file };
    setValue("conferences", updatedConferences);
    trigger(`conferences.${index}.file`);
    onChange?.(hasConferences, updatedConferences);
  };
  ...
  // Handle hasConferences change
  const handleHasConferencesChange = (value: "Yes" | "No") => {
    setValue("hasConferences", value);
    if (value === "No") {
      setValue("conferences", []);
    }
    trigger("hasConferences");
    onChange?.(value, value === "Yes" ? getValues("conferences") : []);
  };
  ...
  const onSubmit = async (data: ConferencesFormData) => {
    ...
    const submissionData: ConferencesPayload = {
      hasConferences: data.hasConferences,
      Conferences: data.hasConferences === "Yes" ? data.conferences : [],
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="border-b border-gray-200 dark:border-[#064e78] pb-4">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  Share conferences or seminars you participated in{" "}
                </p>
              </div>
              ...
              <div className="space-y-4">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                  Have you participated in any conferences or seminars?{" "}
                    ...
                    >
                      <div className="relative">
                        <input
                          type="radio"
                          value={option}
                          checked={hasConferences === option}
                          onChange={() =>
                            handleHasConferencesChange(option as "Yes" | "No")
                          }
                          className="sr-only"
                        />
                        <div
                          className={`w-5 h-5 rounded-sm transition-all duration-300 ${
                            hasConferences === option
                ...
                {errors.hasConferences && (
                  <p className="text-red-500 text-xs mt-1">
                    {errors.hasConferences.message}
                  </p>
                )}
              </div>

              {/* Conditional conference inputs */}
              {hasConferences === "Yes" && (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        <FormInput
                          ...
                          type="text"
                          register={register(`conferences.${index}.title`)}
                          placeholder="Enter conference title"
                          icon={<Icons.Conference className="w-4 h-4" />}
                          error={errors.conferences?.[index]?.title?.message}
                          required
                        />

                        <FileUpload
                          id={`conference-file-${index}`}
                          label="Conference Document"
                          onFileAccepted={(file) =>
                            handleFileChange(index, file)
                          }
                          onFileRemove={() => handleFileRemove(index)}
                          error={errors.conferences?.[index]?.file?.message}
                          currentFile={conferences[index]?.file}
                        />
                      </div>
                    </div>
                  ))}
              ...
              {/* No Conferences Selected Message */}
              {!conferencesSelected && hasConferences === "No" && (
                <div className="text-center py-12 bg-gray-50 dark:bg-[#022b40] rounded-sm">
                  <Icons.Conference className="w-12 h-12 mx-auto text-gray-400 dark:text-gray-600 mb-3" />
                  <p className="text-gray-600 text-sm dark:text-gray-400">
                    You have indicated that you do not have any conferences to
                    share.
                  </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-6 mt-6 border-t border-gray-200 dark:border-[#064e78]">
                ...
                <button
                  ...
                  className={`flex-1 relative group ${
                    !hasConferences ? "opacity-50 cursor-not-allowed" : ""
                  }`}
                >
